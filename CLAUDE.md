...
- Tests: `t/00-load.t` (smoke), `t/01-multinode.t` (functional), `t/node.t` (Node class), `t/tree.t` (Tree container), `t/handle.t` (Handle cursor), `t/bugfix-*.t` (regression tests, one per bug fix), `t/04-edge-cases.t` (edge cases)
...
- **Bug fix tests get individual files**: Each bug fix regression test lives in its own `t/bugfix-<description>.t` file. Do not add new bug fix tests to a shared file — create a new `t/bugfix-*.t` file with a descriptive name (e.g., `t/bugfix-remove-child-cursor.t`)
